...
static void float_thd(void *arg) {
	...
	while (!VESC_IF->should_terminate()) {
		...
		case (RUNNING_FLYWHEEL):
			...
			apply_speedtilt(d);
			apply_turntilt(d);
			// removing this causes the limit current function to turn in to a deathtrap (see commit death trap cuts at wrong time)
			// however thats not my issue, it must be that its actually hitting that function for the first time since i didnt have that issue riding before, so why is it entering the limit current function?
			// apply_torquetilt(d);
